...
const GLOBAL_RULES = {
  ...
  en: `\n\n[GLOBAL RULE]\n- Output ONLY the final result without any conversational filler.`
};
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ success: false, message: '서버에 GEMINI_API_KEY가 설정되지 않았습니다.' });
    }
    ...
    // 구글 v1 정식 Endpoint 및 gemini-2.0-flash 모델 사용
    ...
    res.status(500).json({ success: false, message: 'AI 통신 중 오류가 발생했습니다: ' + error.message });
  }
};